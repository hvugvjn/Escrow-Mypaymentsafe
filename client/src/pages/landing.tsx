...
export default function Landing() {
  ...
  // Show toast if Google login failed (redirected back with ?error=)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const err = params.get("error");
    if (err === "google_failed") {
      toast({ variant: "destructive", title: "Google Sign-In Failed", description: "Could not sign in with Google. Please try again or use email." });
      window.history.replaceState({}, "", window.location.pathname);
    } else if (err === "google_not_configured") {
      toast({ variant: "destructive", title: "Google Sign-In Not Configured", description: "GOOGLE_CLIENT_ID & GOOGLE_CLIENT_SECRET environment variables are missing on the server." });
      window.history.replaceState({}, "", window.location.pathname);
    }
  }, []);
              ...
              {step === "auth" && (
                <motion.form key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleAuth} className="space-y-4">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-3 w-full h-12 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-colors shadow-sm text-sm font-bold text-slate-700 active:scale-[0.99]"
                  >
                  ...
                  <div className="flex items-center gap-3 py-1">
                    ...
                    <span className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider">or continue with email</span>
                    <div className="flex-1 h-px bg-border" />
                  </div>
